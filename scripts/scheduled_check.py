...
run `prepare_one.py`. If `prepare_one` produces a `build/prepared/<...>/`
...
Failures of `prepare_one` itself (e.g. transient git/network errors) are
recorded in the summary and cause the script to exit non-zero, but the
...
from build_request_from_issue import (
...
def probe_pair(repo_root, pkg_path, arch, probe_dir):
    """Run prepare_one for one (pkg, arch). Returns (needs_build, error_or_None)."""
    if probe_dir.exists():
    ...
    cmd = [
        sys.executable,
        str(repo_root / "scripts" / "prepare_one.py"),
    ...
    if result.returncode != 0:
        return False, f"prepare_one exited {result.returncode}"

    # prepare_one writes "<name>-<version>/" into the output dir on success,
    # and nothing on a clean "already up to date" / "arch not declared" exit.
    # _temp_* dirs are cleaned by prepare_one in its finally block.
